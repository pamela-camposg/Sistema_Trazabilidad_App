"""
revisar_consolidado.py — Revisión del consolidado Sur (versión refactorizada)
Ambipar Group — Proyecto TRZ-APP-001, Etapa 1

Mismo patrón que src/rm/revisar_consolidado.py: la lógica de las diez
validaciones (V0 a V9) es exactamente la misma que en
03_revisar_consolidado_sur.py, línea por línea. Lo único que cambia es que
las rutas se reciben como parámetro y la función devuelve los resultados
además de escribir el Excel.

Se puede usar de dos formas:

    Desde otro programa:
        from revisar_consolidado import revisar
        resultado = revisar("C:/ruta/PRUEBA_TRAZABILIDAD_SUR.xlsx")
...
# ══════════════════════════════════════════════════════════════════
# VALIDACIONES — idénticas a 03_revisar_consolidado_sur.py
# ══════════════════════════════════════════════════════════════════
...
def validar_regiones(df, p):
    ...
    if not bad.empty:
        bad.insert(0, "PROBLEMA", "Región no válida para SUR")
...
def revisar(ruta_consolidado, ruta_salida=None, hoja=HOJA_DESTINO, mostrar=True):
    """Revisa un consolidado de Sur y devuelve las alertas encontradas.
    ...
    p(f"Iniciando revisión consolidado SUR...")
...
def _main():
    ...
    if argumentos:
        ...
        salida = ruta.parent / "REVISION_CONSOLIDADO_SUR.xlsx"
    else:
